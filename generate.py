...
DEFAULT_SYSTEM_PROMPT: str = (
    'You are a useful and friendly assistant with great erudition and developed intelligence. You '
    'can keep up a conversation on various topics and even know how to play complex intellectual'
    ' games.')
...
def generate_full_prompt(model: MultimodalModel,
    ...
    if len(previous_dialogue) == 0:
        if len(last_answer) > 0:
            ...
            conversation_logger.error(err_msg)
            raise ValueError(err_msg)
        new_prompt = ('<s>[INST] ' + system_prompt + ' ')
    else:
        if len(last_answer) > 0:
...
def generate_text(model: MultimodalModel,
                  tokenizer: AutoTokenizer,
                  cur_query_list: List[Dict[str, str]],
                  history_list: Tuple[str, str],
                  system_prompt: str) -> Tuple[str, Tuple[str, str]]:
    prompt = generate_full_prompt(model, cur_query_list, history_list, system_prompt)
...
def get_ppl(model: MultimodalModel,
            tokenizer: AutoTokenizer,
